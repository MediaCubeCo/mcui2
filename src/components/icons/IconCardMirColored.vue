<script setup lang="ts">
import McIconShell from '../elements/McIconShell/McIconShell.vue'
import { useId } from 'vue'
import { mcSvgIconStandaloneProps } from '@/composables/useMcSvgIconProps'

const viewBox = '0 0 24 24'
const props = defineProps({ ...mcSvgIconStandaloneProps })
const uid = useId()
function getUrl(id: string) {
  return `url(#${uid}-${id})`
}
</script>
<template>
  <McIconShell :view-box="viewBox" v-bind="props">
    <rect width="24" height="24" rx="12" fill="#3C88FA"/>
    <rect x="3" y="5.25" width="18" height="13.5" rx="5" fill="white"/>
    <path d="M10.9642 13.7258V13.7273C10.9629 13.7273 10.5344 13.7258 10.4202 14.1595C10.3154 14.5572 10.0203 15.6546 10.0121 15.6849H9.93051C9.93051 15.6849 9.62856 14.563 9.52246 14.1581C9.40821 13.7244 8.9784 13.7258 8.9784 13.7258H8V17.0244H8.97886V15.0653H9.06047L9.63173 17.0244H10.3114L10.8826 15.0668H10.9642V17.0244H11.9431V13.7258H10.9642ZM14.5401 13.7258C14.5401 13.7258 14.2531 13.7532 14.1184 14.0716L13.4252 15.6849H13.3436V13.7258H12.3647V17.0244H13.2892C13.2892 17.0244 13.5898 16.9956 13.7244 16.6786L14.4041 15.0653H14.4857V17.0244H15.4645V13.7258H14.5401ZM15.8998 15.2238V17.0244H16.8786V15.9731H17.9391C18.4016 15.9731 18.7928 15.6604 18.9384 15.2243H15.8998V15.2238Z" fill="#1C8258"/>
    <path d="M17.9389 13.7256H15.7617C15.8705 14.3538 16.3153 14.8576 16.8902 15.0219C17.0236 15.0602 17.1613 15.0795 17.2996 15.0795H18.9776C18.9925 15.0046 18.9993 14.9282 18.9993 14.849C18.9993 14.2284 18.5246 13.7256 17.9389 13.7256Z" :fill="getUrl('paint0_linear_14747_37653')"/>
    <defs>
    <linearGradient :id="uid + '-paint0_linear_14747_37653'" x1="15.7617" y1="14.4025" x2="18.9993" y2="14.4025" gradientUnits="userSpaceOnUse">
    <stop offset="0.3" stop-color="#02ADFF"/>
    <stop offset="1" stop-color="#2968EA"/>
    </linearGradient>
    </defs>
  </McIconShell>
</template>
