<script setup lang="ts">
import McIconShell from '../elements/McIconShell/McIconShell.vue'
import { mcSvgIconStandaloneProps } from '@/composables/useMcSvgIconProps'

const viewBox = '0 0 32 33'
const props = defineProps({ ...mcSvgIconStandaloneProps })
</script>
<template>
  <McIconShell :view-box="viewBox" v-bind="props">
    <path d="M28.4643 10.3485C28.1647 9.23467 27.2864 8.35662 26.1728 8.05669C24.1382 7.5 15.9997 7.5 15.9997 7.5C15.9997 7.5 7.86152 7.5 5.82693 8.03559C4.73466 8.3352 3.83503 9.23482 3.53543 10.3485C3 12.383 3 16.602 3 16.602C3 16.602 3 20.8425 3.53543 22.8556C3.83535 23.9693 4.71324 24.8473 5.82709 25.1473C7.88294 25.7041 16 25.7041 16 25.7041C16 25.7041 24.1382 25.7041 26.1728 25.1686C27.2866 24.8688 28.1647 23.9908 28.4646 22.8771C28.9999 20.8425 28.9999 16.6235 28.9999 16.6235C28.9999 16.6235 29.0213 12.383 28.4643 10.3485ZM13.4086 20.4999V12.7043L20.1762 16.602L13.4086 20.4999Z" fill="currentColor" />
  </McIconShell>
</template>
