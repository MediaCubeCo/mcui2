...
const props = defineProps({
   ...
   */
  currentValues: {
    type: Object as PropType<FilterConditionValue>,
    default: () => ({}),
  },
})
...
const isAjax = computed(() => {
  return typeof props.filter.getAjaxOptions === 'function'
})

const computedOptions = computed(() => {
  let options = isAjax.value ? ajaxOptions.value : props.filter.options || []

  if (!helper.isEmpty(props.currentValues) && props.filter.value in props.currentValues) {
    const category = props.currentValues[props.filter.value]
    let selected: IRelationFilter[] = []
...
const setValue = (value: FilterRelationValue): void => {
  ...
  let currentValueName: FilterRelationValue | null = null
  ...
  if (relationType.value === FilterRelations.Exists && !props.currentValues[props.filter.value]) {
    ...
    currentValueName = { [FilterRelations.Exists]: [0] }
  }
  /**
   * Событие по изменению значения
   */

  emit('update:modelValue', { value: currentValue, valueName: currentValueName })
}
